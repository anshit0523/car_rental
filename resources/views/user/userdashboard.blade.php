                        ...
                        <div class="flex justify-between items-start">
                            <div>
                                ...
                                <p class="text-3xl font-bold text-gray-900 mt-2">
                                    &#8369;{{ number_format($totalSpent ?? 0, 2) }}</p>
                            </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                        ...
                        <div class="p-6 space-y-4">
                            @forelse($currentRentals ?? [] as $rental)
                                <div class="flex items-center gap-4 pb-4 border-b border-gray-200 last:border-0">
                                    <div
                                        class="aspect-[16/9] w-32 rounded overflow-hidden bg-gray-200 flex items-center justify-center">
                                        @if($rental->car->images && count(json_decode($rental->car->images)) > 0)
                                            @php
                                                $images = json_decode($rental->car->images);
                                                $firstImage = $images[0];
                                             @endphp
                                            <img src="{{ asset('storage/' . $firstImage) }}" alt="Car Image"
                                    ...
                                    <div class="flex-1">
                                        <h4 class="font-bold text-gray-900">{{ $rental->car->brand->name ?? 'Car' }}
                                            {{ $rental->car->model ?? '' }}
                                        </h4>
                                        <p class="text-sm text-gray-600">Pickup: {{ $rental->pickup_at->format('M d, Y') }} •
                                            Return: {{ $rental->return_at->format('M d, Y') }}</p>
                                    </div>
                                    <div class="text-right">
                                        <p class="font-bold text-gray-900">
                                            &#8369;{{ number_format($rental->total_price, ) }}/day</p>
                                        <span
                                            class="inline-block px-3 py-1 rounded-full text-xs font-medium {{ $rental->status->id == 2 ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                    ...
                    <div class="bg-white rounded-lg shadow-sm">
                        ...
                        <div class="p-6 space-y-3">
                            ...
                            <button
                                class="w-full flex items-center justify-center border border-gray-300 px-4 py-3 rounded-lg font-medium text-gray-700 hover:bg-gray-50 transition">
